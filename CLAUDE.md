...
- [~] **Supabase Cloud verdrahtet**: Projekt `DeEnProjekt` (ref `pqyvphzpajcwwerqbknx`, Frankfurt/eu-central-1) · `.env` mit anon-Key gesetzt (gitignored) · Schema `0001` erweitert (kind/revenue_model/sale_type/monthly_revenue/price_value/listed_at/equity + Indizes) · Seed auto-generiert aus `listings.ts` (`scripts/gen-seed.mjs` → `supabase/seed.sql`) · App lädt via `src/data/listingsRepo.ts` aus Supabase **mit Fallback auf statische Daten** (Seite nie leer)
- [ ] Owner-Schritt: `supabase/setup.sql` im Supabase SQL-Editor ausführen (Schema+Seed, idempotent) · danach `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` als Env-Vars in Cloudflare Pages + Redeploy
